            ...
            for info in object.contains:
                if info.codeName() == 'dmap.contentcodesnumber':
                    ...
                    try:
                        ...
                        logger.debug('DAAPParseCodeTypes: unknown data type %s for code %s, defaulting to s', info.value, name)
                        dtype   = 's'
                else:
                    raise DAAPError('DAAPParseCodeTypes: unexpected code %s at level 2' % info.codeName())
            if code == None or name == None or dtype == None:
                logger.debug('DAAPParseCodeTypes: missing information, not adding entry')
            else:
                try:
                    dtype = dmapFudgeDataTypes[name]
                except: pass
                #print("** %s %s %s", code, name, dtype)
                if code not in dmapCodeTypes:
                    print code
...
class DAAPObject(object):
    ...
    def encode(self):
        ...
        if self.type == 'c':
            ...
            try:
                ...
                raise ValueError("Error while packing code '%s': %s", self.code, e)
            return data
    ...
    def processData(self, str):
        ...
        if self.type == 'l':
            ...
            logger.debug('DAAPObject: Unknown code %s for type %s, writing raw data', code, self.code)
            self.value  = code
